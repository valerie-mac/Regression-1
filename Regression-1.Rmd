...
Let's do the regression now on the training set. Linear regression is performed with the R function "lm" and takes the form,object name helps us to save the model 
...
```{r}
# linear regression of child height on mid-parent height in the training dataset- the ~separates the outcome from the predictors y~x="formula"
...
```{r}
#set up the 0 line- created a new variable called parent, and adding lables to the parent axis - adding a vector of 64 to 73 in increments of 0.01, then create resid for the residuals on the y
x0 <- 64
parent <- seq(x0, 73, 0.01)
resid <- rep(0, length(parent))

#plot residual versus X - abline(0,0) gives us a line at that slope and intercept
plot(resid~parent, data = Galton.test)
abline(0,0)
#overlay the zero line -need the labels below so that its a better residual plot#this means i will be able to overlay
par(new=TRUE)
...
```{r results="asis"}
# compare the results of the two regression models if you want it to look pretty do type=html when using stargazer
...
We also want to look for multicollinearity, that is are some of our independent variables highly correlated. We do this by looking at the Variance Inflation Factor (VIF). A GVIF > 4 suggests collinearity. This is the amount of multicollinearity that this value has with he other 2 variables. f the VIF of a variable is higher than 4, consider removing that variable because it is hightly correlated with something else.
